...
export async function GET(_: Request, context: { params: Promise<{ orderId: string }> }) {
  ...
  const order = await env.DB.prepare("SELECT id,status,total,delivery_address,created_at,updated_at FROM orders WHERE id = ?").bind(orderId).first();
  if (!order) return Response.json({ error: "Order not found" }, { status: 404 });
  const delivery = await env.DB.prepare("SELECT id,status,pickup_address,dropoff_address,distance_km,estimated_arrival,picked_up_at,delivered_at,current_lat,current_lng,location_updated_at FROM deliveries WHERE order_id = ?").bind(orderId).first();
