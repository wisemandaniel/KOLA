...
import { AccountCenter, AdminHome, PaymentCenter } from "./PlatformCenter";
...
type Workspace = { products:Product[];orders:Order[];deliveries:Delivery[];messages:Message[];addresses:Address[];actor?:{id:string;displayName:string;activeRole:Role;city?:string;phone?:string;authProvider?:"whatsapp";vendorId?:string;vendorSlug?:string;isAdmin?:boolean;adminLevel?:"none"|"admin"|"superadmin"} };
  ...
  return <main className="app-frame">
    ...
    <section className="app-main">
      ...
      <div className="app-content">
        ...
        {tab==="Home"&&(role==="admin"||role==="superadmin")&&<AdminHome onNotice={showNotice}/>}
...
function Sidebar({role,tab,setTab,openChat}:{role:Role;tab:Tab;setTab:(t:Tab)=>void;openChat:()=>void}){
  return <aside className="app-sidebar"><Logo/><div className="workspace-label"><span>{role==="superadmin"?"Superadmin console":role==="admin"?"Admin console":role==="vendor"?"Business workspace":role==="rider"?"Rider workspace":"Customer account"}</span></div><nav>{navItems.map(([label,icon])=><button key={label} className={tab===label?"active":""} onClick={()=>label==="Messages"?openChat():setTab(label)}>{icon}<span>{label}</span></button>)}</nav><div className="sidebar-help"><MessageCircle/><div><b>Need help?</b><span>Contact Kola support</span></div><ChevronRight/></div></aside>
}
...
function AppHeader({actor,role,cartCount,onCart,onNotifications}:{actor:Workspace["actor"];role:Role;cartCount:number;onCart:()=>void;onNotifications:()=>void}){return <header className="app-header"><button className="mobile-menu" aria-label="Menu"><Menu/></button><div className="header-location"><MapPin/><div><span>{role==="customer"?"Delivering to":role==="admin"||role==="superadmin"?"Platform operations":"Operating in"}</span><b>{actor?.city||"Cameroon"}</b></div></div><div className="header-actions">{role==="customer"&&<button className="cart-button" onClick={onCart}><ShoppingBag/><span>Bag</span>{cartCount>0&&<b>{cartCount}</b>}</button>}<button className="icon-button" aria-label="Notifications" onClick={onNotifications}><Bell/></button><div className="header-profile"><span>{actor?.displayName.split(" ").map(p=>p[0]).join("").slice(0,2)}</span><div><b>{actor?.displayName}</b><small>{role==="superadmin"?"Superadmin":role==="admin"?"Administrator":role==="vendor"?"Business":role==="rider"?"Delivery rider":"Customer"}</small></div></div></div></header>}
...
function PaymentsPage({data,role,onNotice}:{data:Workspace;role:Role;onNotice:(message:string)=>void}){const paid=data.orders.filter(o=>o.payment_status==="paid").reduce((s,o)=>s+Number(o.total),0);if(role==="admin"||role==="superadmin")return <AdminHome onNotice={onNotice}/>;return <><PageTitle eyebrow="FINANCE" title={role==="customer"?"Payments":"Settlements"} subtitle="Cash on delivery and secure Fapshi Mobile Money checkout."/><div className="metrics payments"><Metric label="Confirmed" value={`${paid.toLocaleString()} FCFA`} note="Recorded paid orders"/><Metric label="Pending cash orders" value={String(data.orders.filter(o=>o.payment_status==="pending").length)} note="Collected on delivery"/><Metric label="Payment methods" value="2" note="Cash and Fapshi Mobile Money"/></div>{role==="customer"?<PaymentCenter orders={data.orders} onNotice={onNotice}/>:<section className="surface"><Empty icon={<WalletCards/>} title="Settlements overview" text="Completed delivery and sales earnings are included in your Account analytics."/></section>}</>}
function AccountPage({data,actor,role,initialSection,onNotice}:{data:Workspace;actor:Workspace["actor"];role:Role;initialSection:string;onNotice:(message:string)=>void}){const label=role==="superadmin"?"Superadmin account":role==="admin"?"Administrator account":role==="vendor"?"Business account":role==="rider"?"Delivery rider account":"Customer account";return <><PageTitle eyebrow="ACCOUNT" title="Account and operations" subtitle="Manage profile, notifications, support, integrations and permissions."/><section className="profile-surface"><div className="profile-large">{actor?.displayName.split(" ").map(p=>p[0]).join("").slice(0,2)}</div><div><h2>{actor?.displayName}</h2><p>{label} · {actor?.city||"Cameroon"}</p></div><a className="secondary-button" href="/api/auth/logout?return_to=%2F">Sign out</a></section><AccountCenter key={initialSection} role={role} orders={data.orders} initialSection={initialSection} onNotice={onNotice}/></>}
